...
// Booking pause window: while "now" falls between these two dates, the booking
// card is swapped for a short "temporarily unavailable" notice instead.
// This is just a plain browser-clock check that re-runs on every page load, so
...
const BOOKING_PAUSED_UNTIL = new Date("2026-09-30T00:00:00");

function isBookingPaused(): boolean {
  const now = new Date();
  return now >= BOOKING_PAUSED_FROM && now < BOOKING_PAUSED_UNTIL;
}
...
export function EmotionalSupport() {
  ...
  const bookingPaused = isBookingPaused();
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        ...
        <Card className="mb-8 border-2 border-purple-200 dark:border-purple-800">
          <CardHeader>
            <CardTitle className="text-2xl font-bold">{t("emotionalSupport.booking.pausedTitle")}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-gray-600 dark:text-gray-300">{t("emotionalSupport.booking.pausedText")}</p>
          </CardContent>
        </Card>
        ) : (
        <Card className="mb-8 border-2 border-purple-200 dark:border-purple-800">
