...
import { AdminOtp, AdminSession, RateLimitHit } from "../src/server/models/AdminAuth.js";
...
// admin-dashboard's standalone production origin (e.g. https://admin.nuppuapp.fi),
// once it's deployed there as its own Vercel project - needed because that's a
// genuinely cross-origin request, unlike the legacy same-origin /admin path.
// Unset is fine before that cutover; local dev is already covered below.
...
const allowedOrigins = new Set(
  [
    ...
    "http://localhost:5173",
    "http://127.0.0.1:5173",
    // admin-dashboard's dev server - Vite bumps to 5174 when 5173 is taken.
    "http://localhost:5174",
    "http://127.0.0.1:5174",
    ADMIN_DASHBOARD_URL,
  ].filter(Boolean),
...
// belt-and-suspenders - Vercel already sets these at the edge for the static frontend
// (see vercel.json / admin-dashboard/vercel.json, which must be kept in sync with the
// CSP below), but this covers direct API hits, JSON error pages, and non-Vercel
// deployments where this same Express app serves the built static site itself.
//
// script-src has no 'unsafe-inline'/'unsafe-eval' - the Vite build never needs them.
// style-src keeps 'unsafe-inline' because React's style={{...}} prop and Tailwind's
// generated <style> tag both render as inline styles; that's a much smaller XSS
// surface than allowing inline <script>, which is what actually matters for an
// admin token sitting in sessionStorage.
...
// createRateLimiter is a hoisted function declaration (defined further down),
// so it's safe to call here even though this line runs first.
const rateLimitContact = createRateLimiter({ name: "contact", windowMs: 60_000, max: 5 });

// Fallback-only, per-process admin OTP + session state, used when Mongo isn't
// connected (e.g. local dev without a database running). When Mongo is
// connected the AdminOtp/AdminSession models below are authoritative instead -
// see src/server/models/AdminAuth.js for why (shared/durable across serverless
// instances, unlike a plain Map).
const adminOtpStore = new Map(); // email -> { code, expiresAt, attempts } - Mongo-down fallback only
const adminSessions = new Map(); // sessionToken -> { email, expiresAt } - Mongo-down fallback only
...
async function setAdminOtp(email, { code, expiresAt }) {
  ...
  adminOtpStore.set(email, { code, expiresAt: expiresAt.getTime(), attempts: 0 });
}
...
async function getAdminOtp(email) {
  ...
  return adminOtpStore.get(email) ?? null;
}
...
async function incrementAdminOtpAttempts(email) {
  if (await ensureMongoConnection()) {
    ...
    return updated?.attempts ?? Infinity;
  }
...
async function requireAdmin(req, res, next) {
  ...
  if (ADMIN_TOKEN && timingSafeStringEqual(providedToken, ADMIN_TOKEN)) {
    return next();
  }

  try {
    const session = await getValidAdminSession(providedToken);
    if (session) {
      return next();
    }
  } catch (error) {
...
// Sliding-window rate limiter, backed by the RateLimitHit collection so limits
// are shared across serverless instances instead of being per-process (see
// src/server/models/AdminAuth.js). Falls back to an in-process Map when Mongo
// isn't connected, same as the admin OTP/session store above. `name` scopes the
// bucket key so independent limiters (contact, OTP, payments, admin, ...) never
// collide with each other.
function createRateLimiter({ name, windowMs, max }) {
  const memoryState = new Map();

  function checkMemory(key, now) {
    const data = memoryState.get(key) ?? { count: 0, startedAt: now };
    ...
    if (await ensureMongoConnection()) {
      try {
        ...
        const hit = await RateLimitHit.findOneAndUpdate(
          { key: bucketKey },
          { $inc: { count: 1 }, $setOnInsert: { expiresAt: new Date(now + windowMs + 60_000) } },
          { upsert: true, returnDocument: "after" },
        ).lean();
        count = hit.count;
      } catch (error) {
        console.error(`Rate limiter (${name}) DB error, falling back to in-memory:`, error);
        count = checkMemory(key, now);
      }
    } else {
      count = checkMemory(key, now);
    }
...
// generous but real - protects admin-token/OTP-session guessing from being
// unlimited even though ADMIN_TOKEN itself is a long random secret
const rateLimitAdmin = createRateLimiter({ name: "admin", windowMs: 60_000, max: 60 });

async function requireCronOrAdmin(req, res, next) {
  const authHeader = req.header("authorization") ?? "";
  const bearerMatch = authHeader.match(/^Bearer\s+(.+)$/i);

  if (CRON_SECRET && bearerMatch && timingSafeStringEqual(bearerMatch[1], CRON_SECRET)) {
    return next();
  }
...
// Limits are much higher outside production so iterating on the sign-in flow
// locally doesn't repeatedly trip the same 15-minute window - real brute-force
// protection only needs to be tight where it matters.
const rateLimitOtpRequest = createRateLimiter({
  name: "otp-request",
  windowMs: 15 * 60_000,
  max: NODE_ENV === "production" ? 10 : 200,
});
const rateLimitOtpVerify = createRateLimiter({
  name: "otp-verify",
  windowMs: 15 * 60_000,
  max: NODE_ENV === "production" ? 20 : 400,
});
...
app.post("/api/admin/otp/request", rateLimitOtpRequest, async (req, res) => {
  ...
  await setAdminOtp(email, { code, expiresAt: new Date(Date.now() + ADMIN_OTP_TTL_MS) });
...
app.post("/api/admin/otp/verify", rateLimitOtpVerify, async (req, res) => {
  const email = String(req.body?.email ?? "").trim().toLowerCase();
  const code = String(req.body?.code ?? "").trim();

  const entry = await getAdminOtp(email);
  if (!entry || entry.expiresAt < Date.now()) {
    await deleteAdminOtp(email);
    return res.status(401).json({ status: "error", message: "That code is invalid or has expired." });
  }

  const attempts = await incrementAdminOtpAttempts(email);
  if (attempts > ADMIN_OTP_MAX_ATTEMPTS) {
    await deleteAdminOtp(email);
    return res.status(401).json({ status: "error", message: "Too many attempts. Request a new code." });
  }

  if (!timingSafeStringEqual(code, entry.code)) {
    return res.status(401).json({ status: "error", message: "That code is invalid or has expired." });
  }

  await deleteAdminOtp(email); // one-time use
...
app.get("/api/contact", rateLimitAdmin, requireAdmin, requireDatabase, async (req, res) => {
  try {
...
app.patch("/api/contact/:id", rateLimitAdmin, requireAdmin, requireDatabase, async (req, res) => {
  try {
    const { id } = req.params;
...
app.delete("/api/contact/:id", rateLimitAdmin, requireAdmin, requireDatabase, async (req, res) => {
  try {
...
app.delete("/api/contact", rateLimitAdmin, requireAdmin, requireDatabase, async (req, res) => {
  try {
...
const rateLimitPayments = createRateLimiter({ name: "payments-create", windowMs: 60_000, max: 10 });
...
const rateLimitAvailability = createRateLimiter({ name: "availability-slots", windowMs: 60_000, max: 30 });
...
app.get("/api/availability/calendar", rateLimitAdmin, requireAdmin, requireDatabase, async (req, res) => {
  try {
    const days = await computeSlotStatuses({ from: req.query.from, to: req.query.to });
...
app.post("/api/availability/blocks", rateLimitAdmin, requireAdmin, requireDatabase, async (req, res) => {
  try {
...
app.delete("/api/availability/blocks/:id", rateLimitAdmin, requireAdmin, requireDatabase, async (req, res) => {
  try {
...
app.post("/api/payments/create", rateLimitPayments, requireDatabase, async (req, res) => {
  try {
    ...
    const paytrailPayment = await createPaytrailPayment({
      ...
      reference,
      amountCents,
      description: "Nuppu emotional support consultation (30 min)",
...
app.get("/api/payments/export", rateLimitAdmin, requireAdmin, requireDatabase, async (req, res) => {
  try {
...
app.get("/api/payments/anonymize-expired", rateLimitAdmin, requireCronOrAdmin, requireDatabase, async (req, res) => {
  try {
...
app.get("/api/payments", rateLimitAdmin, requireAdmin, requireDatabase, async (req, res) => {
  try {
    const { status, limit = 50, page = 1 } = req.query;
...
app.get("/api/payments/:id", rateLimitAdmin, requireAdmin, requireDatabase, async (req, res) => {
  try {
...
app.patch("/api/payments/:id", rateLimitAdmin, requireAdmin, requireDatabase, async (req, res) => {
  try {
...
app.delete("/api/payments/:id/personal-data", rateLimitAdmin, requireAdmin, requireDatabase, async (req, res) => {
  try {
