  ...
  return (
    <MobileScreen>
      <div className="flex-1 overflow-y-auto bg-gradient-to-br from-[#D4C5F9]/30 via-[#A8D5E2]/15 to-white px-6 py-8">
        ...
        <h1 className="text-2xl font-bold text-[#2D3748] mb-1 mt-6">Create your child's profile</h1>
        <p className="text-sm text-[#718096] mb-6">Let's personalize their experience</p>

        <p className="text-sm font-semibold text-[#4A5568] mb-3">Choose an avatar</p>
          ...
          {(Object.keys(AVATAR_MAP) as AvatarId[]).map((id) => (
            <button
              ...
              className={`aspect-square rounded-2xl overflow-hidden transition-all ${
                avatar === id ? 'ring-4 ring-[#6B9AC4] scale-110' : 'ring-1 ring-gray-200'
              }`}
            >
              <ImageWithFallback
        ...
        <label className="text-xs text-[#6B9AC4] font-medium cursor-pointer mb-6 inline-block">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-[#2D3748]">
                    {group.label} <span className="text-[#718096] font-normal">· {group.range}</span>
                  </p>
                  <p className="text-xs text-[#718096] mt-0.5">{group.description}</p>
                </div>
                <span
                  className={`w-5 h-5 rounded-full border-2 shrink-0 ${
                    selected ? 'border-[#6B9AC4] bg-[#6B9AC4]' : 'border-gray-300'
                  }`}
                />
              </button>
            );
        ...
        <p className="text-sm font-semibold text-[#4A5568] mb-3">What do they love?</p>
        ...
        <div className="flex gap-2 mb-6">
          ...
          <button
            ...
            className="shrink-0 w-12 h-12 rounded-xl bg-[#A8D5E2]/25 text-[#6B9AC4] flex items-center justify-center"
        ...
        <p className="text-sm font-semibold text-[#4A5568] mb-1">Anything specific to include? (optional)</p>
        <Input
        ...
        <div className="flex items-start gap-2 mt-3 mb-8 bg-[#B8DDB8]/15 border border-[#B8DDB8]/60 rounded-xl p-3">
          <ShieldCheck className="w-4 h-4 text-[#4A5568] shrink-0 mt-0.5" />
  ...
  return (
    <div className="flex gap-2">
      {Array.from({ length: total }, (_, i) => i + 1).map((s) => (
        <div
          key={s}
          className={`h-1.5 flex-1 rounded-full ${s <= step ? 'bg-[#6B9AC4]' : 'bg-gray-200'}`}
        />
      ))}
    </div>
  );
