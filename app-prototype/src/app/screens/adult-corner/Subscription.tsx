    ...
    <MobileScreen>
      <div className="flex-1 overflow-y-auto bg-gradient-to-br from-[#D4C5F9]/25 via-[#A8D5E2]/10 to-white">
        ...
        <div className="px-6 py-6">
              ...
              return (
                <button
                  ...
                  className={`text-left rounded-2xl border-2 p-5 transition-all ${
                    active ? 'border-[#6B9AC4] bg-[#A8D5E2]/10 shadow-md' : 'border-gray-200 bg-white'
                  }`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-base font-bold text-[#2D3748]">{p.name}</p>
                    {active && (
                      <span className="text-[10px] font-bold text-white bg-[#6B9AC4] px-2 py-1 rounded-full">
                        CURRENT
                      </span>
                    )}
                  </div>
                  <p className="text-sm font-semibold text-[#6B9AC4] mb-3">{p.price}</p>
                  <ul className="space-y-2">
                    {p.features.map((feature) => (
                      <li key={feature} className="flex items-start gap-2 text-xs text-[#4A5568]">
                        <Check className="w-3.5 h-3.5 text-[#6B9AC4] shrink-0 mt-0.5" />
                        <span>{feature}</span>
                      </li>
                    ))}
                  </ul>
          ...
          <p className="text-[11px] text-[#718096] text-center mt-6 leading-relaxed">
